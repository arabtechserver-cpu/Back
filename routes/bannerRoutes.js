...
router.post('/', authMiddleware, async (req, res) => {
  const { title, highlight, desc, badge, color, icon, link } = req.body;
  ...
  try {
    ...
    const result = await runQuery(
      ...
      [title, highlight || '', desc || '', badge || '', color || '#8b5cf6', finalIcon, link || '']
    );
    res.status(201).json({
...
router.put('/:id', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'تم تحديث شريحة البانر بنجاح.', id, title, highlight, desc, badge, color, icon: finalIcon, link: link || '' });
  } catch (error) {
